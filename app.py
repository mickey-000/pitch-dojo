...
【評価対象】
相手: {li["name"]}（{li["perspective"]}視点）
テーマ: {theme}
シチュエーション: {ti['situation'][level]}
...
- 採点は必ず各項目の定義に忠実に行い、全体の印象で引き下げてはならない。
- Sランク（90点以上）は各項目がほぼ満点水準のときのみ与える。ただし、明確な根拠なく減点してはならない。

【Sランク参考例（採点の錨として使え）】
以下はSランク相当の発言例である。採点時にこれと比較して各項目を判定せよ。
発言例（納期遅延・部長向け）:
「部長、今四半期4件の遅延、いずれも調達部の入荷遅れが製造部・品管部へ波及する同じパターンです。
原因はExcel・ホワイトボード・紙台帳とバラバラな管理で遅延が即座に共有されないことです。
3部門の進捗を一画面で管理し、入荷遅れが発生した瞬間に全部門へ即時通知が届く仕組みをご提案します。
つなぎ目の遅れをゼロにし、来四半期の遅延0件を実現します。」
→ フック25・施策25・根拠25・着地25＝100点
※施策25点の根拠：ツール名・製品名は不要。「何が起きたら・誰に・何が届くか」という仕組みの動作が具体的に示されていれば満点。
...
■ 施策の具体性（25点）
25点: 「何を・どうするか」が具体的に示されており、相手が行動イメージを持てる（1〜2施策）
　※ツール名・製品名は不要。「3部門共通の進捗を一画面で管理し、遅延発生時に全員へ即時通知する」など、解決の仕組みや動作が伝わればよい
　※「誰が・何を・どうする」の構造が明確であれば25点
20点: 解決の方向性は伝わるが、どう動くか・何が変わるかの描写がやや薄い
15点: やりたいことは伝わるが手段・方法が不明確
10点: バズワード止まり（「見える化」「効率化」「DX推進」「リアルタイム共有」のみで動作説明なし）
5点: 一般論（「改善する」「対策を打つ」）
0点: 施策への言及なし
    ...
    for attempt in range(3):
        try:
            resp = openai_client.chat.completions.create(
                model="gpt-4o-mini",
                messages=[
                    {"role": "system", "content": "あなたは提案力道場の師匠です。指示された評価基準に従い、必ずJSON形式のみで返答してください。"},
                    {"role": "user", "content": prompt}
                ],
                response_format={"type": "json_object"},
                temperature=0.3,
            )
            cj = resp.choices[0].message.content.strip()
            r = json.loads(cj)
